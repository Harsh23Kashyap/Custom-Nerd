...
def _to_unix_timestamp(date_str):
    """Convert ISO date (YYYY-MM-DD) to Unix timestamp for Stack Exchange todate."""
    if not date_str:
        return None
    try:
...
def collect_articles(
    ...
    """
    Stack Exchange (Stack Overflow) answers only.

    AWS / Azure / CloudFest RSS and blog search are not merged into results
    (``cloud_links_per_source`` is ignored).

    Parameters
    ----------
    query_list            : list[str]  — search queries
    max_date              : str | None — ISO date (YYYY-MM-DD); posts after this cutoff
                                         are excluded via Stack Exchange ``todate`` and
                                         a post-fetch answer filter.
    answer_counter        : int        — maximum Stack Overflow answers to return
    cloud_links_per_source: int        — deprecated; kept for call-site compatibility

    Returns up to ``answer_counter`` Stack Overflow answer records.
    """
    try:
        answer_cap = int(answer_counter)
    except (TypeError, ValueError):
        answer_cap = 50

    todate_ts = _to_unix_timestamp(max_date)

    print(
        "[user_search_apis] collect_articles start (Stack Overflow only) — "
        f"queries={len([q for q in query_list if str(q).strip()])}, "
        f"max_date={max_date!r}, answer_counter={answer_cap}"
    )
            ...
            for q in questions:
                ...
                if todate_ts:
                    ...
                    answers = [
                        ...
                        if not (
                            isinstance(ans.get("creation_date"), int)
                            and ans["creation_date"] > todate_ts
                        )
                    ]
                    print(
                        f"    [QID {qid}] answers before cutoff: {before}, "
                        f"after: {len(answers)}"
                    )
